...
@allure.epic('Get/api/v1/analytics/payments_analytics/{category_id}/completed_detail - '
             'Запрос аналитики по исполненным регулярным платежам для заданой категории - проверка поля year from')
class TestAnalyticsCompletedDetailYearFrom:

    @allure.description('проверка поля year from - Значение  = 2020')
