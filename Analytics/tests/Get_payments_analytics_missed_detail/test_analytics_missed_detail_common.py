...
@allure.epic('Get/api/v1/analytics/payments_analytics/{category_id}/missed_detail - Запрос аналитики по пропущенным'
             'регулярным платежам для заданной категории - общие проверки')
class TestAnalyticsMissedDetailCommon:
    ...
    def test_01(self, create_moneybox_and_delete_for_analytics):  # Дата текущий месяц
        ...
        try:
            ...
            PaymentInfoMethods.create_payment_info_in_bd(
                regular_outcome_id, 10, '2025-06-20', None, False, 111
            )  # Дата текущий месяц
    ...
    @allure.description('Запросить аналитику по регулярным платежам с параметрами авторизованный пользователь')
    ...
    @allure.description('Запросить аналитику по единоразовым платежам c параметрами авторизованный пользователь')
    ...
    @allure.description('Запросить аналитику одновременно указав регулярные и единоразовые платежи')
    ...
    @allure.description('Запросить аналитику по платежам указав значение false регулярным и единоразовым платежам')
    def test_06(self, create_moneybox_and_delete_for_analytics):
        ...
        try:
            ...
            get_analytics = AnalyticsMethods.get_payment_analytics_missed_detail_with_params(
                20,
                '?month_from=1&year_from=2025&month_to=12&year_to=2031&regular_payment=false&onetime_payment=false',
                access_token
            )

            """Проверка статус кода"""
            Checking.check_statuscode(get_analytics, 400)
        except AssertionError as e:
            with allure.step(f'Ошибка проверки: {e}'):
                # Подробное описание ошибки
                allure.attach(str(e), attachment_type=allure.attachment_type.TEXT)
    ...
    @allure.description('Запросить аналитику по  платежам  неавторизованный пользователь')
    def test_07(self, create_moneybox_and_delete_for_analytics):
        ...
        try:
            ...
            """Запрос аналитики"""
            get_analytics = AnalyticsMethods.get_payment_analytics_missed_detail_with_params_without_auth(
                20,
                '?month_from=1&year_from=2025&month_to=12&year_to=2031&regular_payment=true&onetime_payment=true'
            )

            """Проверка статус кода"""
            Checking.check_statuscode(get_analytics, 401)
        except AssertionError as e:
            with allure.step(f'Ошибка проверки: {e}'):
                # Подробное описание ошибки
                allure.attach(str(e), attachment_type=allure.attachment_type.TEXT)
